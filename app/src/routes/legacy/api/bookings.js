...
const db = require('../../../../db');
const bookingsModel = require('../../../models/legacy/bookings');
const roomsModel = require('../../../models/legacy/rooms');
...
router.get('/:id', authMiddleware, [ param('id').isInt({ gt: 0 }) ], async (req, res, next) => {
  try {
    ...
    // ตรวจสอบว่ามีใบเสร็จสำหรับการจองนี้หรือไม่
    let receipt = null;
    if (booking.payment_status === 'confirmed') {
      try {
        ...
        // หาไฟล์ใบเสร็จที่มี booking_id นี้
        if (fs.existsSync(receiptsDir)) {
          const files = fs.readdirSync(receiptsDir)
            .filter(file => file.includes(`booking_${booking.booking_id}_`) || file.includes(`_${booking.booking_id}_`));
          
          if (files.length > 0) {
            const latestFile = files.sort().pop(); // ใช้ไฟล์ล่าสุด
            receipt = {
              fileName: latestFile,
              downloadUrl: `/receipts/${latestFile}`,
              directUrl: `/receipts/${latestFile}`
            };
          }
        }
      } catch (receiptError) {
        console.log('ไม่พบใบเสร็จสำหรับการจอง:', booking.booking_id);
      }
    }
...
// สร้างการชำระเงิน (ปรับปรุงให้สร้างใบเสร็จ)
router.post('/:id/payment', authMiddleware, [
  ...
  try {
    ...
    if (method === 'credit_card') {
      // สำหรับบัตรเครดิต - ใช้ Stripe
      const MainPaymentService = require('../../../services/PaymentService');
      const paymentIntent = await MainPaymentService.createPaymentIntent(booking_id, req.user.user_id);
      ...
      res.json({
        ...
        message: 'กรุณาทำการชำระเงินผ่าน Stripe'
      });
      return;
      
    } else {
      // สำหรับวิธีอื่นๆ (เงินสด, โอนเงิน, QR) - ใช้ระบบทั่วไป
      result = await PaymentService.processDirectPayment(
        booking_id,
        method,
        transaction_id,
        req.user.user_id
